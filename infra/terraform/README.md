...
AWS (EKS) is implemented today; GCP and Azure are planned as sibling directories.
For **local** (non-cloud) GPU testing on your own machine, see
[`infra/local/`](../local) — a minikube single-node cluster running the same
KEDA + keda-gpu-scaler stack against a local NVIDIA GPU.
