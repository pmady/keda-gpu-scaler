...
// collectMIGInstances enumerates MIG compute instances on a physical GPU
// and returns one Metrics per instance. Shared chip-level metrics (temp, power,
// PCIe, NVLink) come from the physical handle.
...
// collectMIGDevice reads per-instance metrics from a MIG handle and overlays
// the shared physical metrics.
...
// collectPhysicalForMIG reads chip-level metrics shared across all MIG instances.
