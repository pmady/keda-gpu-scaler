...
and adapter delay typical of the Prometheus pipeline, while supporting GPU
scale-to-zero.
\end{abstract}
...
Device memory is a better proxy, because the KV cache grows with the number and length of
in-flight requests, so VRAM pressure rises ahead of latency. Where the serving engine
exposes its own queue the signal is better still: vLLM's pending-request count and KV-cache
fraction, or Triton's queue wait, measure demand directly rather than inferring it from
hardware state. Training is the case where SM utilization is the right signal, since the
devices are meant to run hot and evenly.
...
SM utilization can stay moderate under heavy load while device memory tracks
request-queue depth closely, so the \texttt{vllm-inference} profile scales on
memory and on the engine's own queue metrics rather than on SM utilization
alone. Scale-to-zero targets development and staging endpoints that sit idle
for long stretches. KEDA's polling interval determines how quickly a traffic
