...
inference engines such as vLLM and Triton, and compare the resulting metric
latency of 2--4~s with the 15--30~s typical of the Prometheus pipeline, while
supporting GPU scale-to-zero.
\end{abstract}
...
for long stretches. The short poll interval determines how quickly a traffic
burst becomes a scaling decision: with the Prometheus pipeline a burst can
saturate GPUs for 15--30~s before a new pod is scheduled, whereas direct NVML
surfaces it within one poll cycle. The best aggregation strategy is
